...
function setup() {
  ...
  //creating coins
  /*coin = createSprite(random(displayWidth - 400, displayWidth - 1300), 
  random(displayHeight - 100, displayHeight - 600), 40, 40);
  coin.addImage("coin", cin);
  coin.scale = 0.2;

  //creating fruits
  fruit = createSprite(random(displayWidth - 400, displayWidth - 1300), 
  random(displayHeight - 200, displayHeight - 600), 40, 40);
...
function draw() {
  ...
  text("Exit", displayWidth - 1306, displayHeight - 703);
    ...
    if(life === 0){
      ...
      textFont("Century Schoolbook");
      stroke("black");
      text("You lose!! Press R to restart", displayWidth / 2, displayHeight - 710);
    }
    if(keyCode === 82){
      gameState = 0;
      player.x = displayWidth/2 + 500;
